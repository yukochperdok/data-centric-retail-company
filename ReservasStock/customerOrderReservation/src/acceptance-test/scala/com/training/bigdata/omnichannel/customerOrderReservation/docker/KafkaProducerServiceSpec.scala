...
import com.training.bigdata.omnichannel.customerOrderReservation.utils.tag.TagTest.AcceptanceTestsSuite
...
class KafkaProducerServiceSpec extends FlatSpec with Matchers with DockerTestKit with DockerKitDockerJava
  with DockerKafkaService {

  implicit val pc = PatienceConfig(Span(20, Seconds), Span(1, Second))

  "kafka container" should "be ready" taggedAs AcceptanceTestsSuite in {
    isContainerReady(kafkaContainer).futureValue shouldBe true
  }

  "kafka" should "be ready" taggedAs AcceptanceTestsSuite in {
