...
trait DockerKuduService extends DockerKit {
  ...
  val kuduContainer: DockerContainer = DockerContainer(
    image = "usuresearch/kudu-docker-slim:latest",
    name = Some("apache-kudu")
  )
  ...
  def restartService: Future[Unit] = containerManager.stopRmAll()
