package com.training.bigdata.arquitectura.ingestion.datalake.streams.reader
...
class KafkaStreamReader(appConfig: AppConfig, ssc: StreamingContext) {
  ...
  def readStream(): InputDStream[ConsumerRecord[String, String]] = {
    ...
    val topics = appConfig.application.topic.map(topic=>topic.name)
    ...
    val additionalSecuredParams = kerberos match {
      case true =>
        Map(
