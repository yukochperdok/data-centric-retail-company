  ...
  def getTimestamp(value: Any) : Long = {
    val milliseconds = value match {
      ...
      case string: String => string match {
        ...
        case _ => throw new DataException(s"String '${value}' cannot be converted to timestamp")
      }
      case _ => throw new DataException(s"Value '${value}' cannot be converted to timestamp")
    }
